...
describe("agpConfig", () => {
  test("listProfiles on empty dir returns [] and creates agp.yaml", async () => {
    await withTempDir(async (dir) => {
      ...
      expect(await Bun.file(join(dir, "agp.yaml")).exists()).toBe(true);
    });
  });

  test("addProfile then listProfiles returns the profile", async () => {
    await withTempDir(async (_dir) => {
      const { addProfile, listProfiles } = await import("../../src/lib/agpConfig");
      await addProfile({ name: "test", description: "Test profile", created: "2026-01-01T00:00:00Z" });
      const profiles = await listProfiles();
  ...
  test("configProfileExists returns false for missing, true after add", async () => {
    await withTempDir(async (_dir) => {
      const { configProfileExists, addProfile } = await import("../../src/lib/agpConfig");
      expect(await configProfileExists("ghost")).toBe(false);
      await addProfile({ name: "ghost", description: "", created: "2026-01-01T00:00:00Z" });
  ...
  test("removeProfile removes from list and persists", async () => {
    await withTempDir(async (dir) => {
      const { addProfile, removeProfile, listProfiles } = await import("../../src/lib/agpConfig");
      await addProfile({ name: "alpha", description: "", created: "2026-01-01T00:00:00Z" });
      await addProfile({ name: "beta", description: "", created: "2026-01-01T00:00:00Z" });
