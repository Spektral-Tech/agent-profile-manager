...
describe("agp usage", () => {
  ...
  test("shows usage summary with sessions", async () => {
    await withTempProfiles(async (dir) => {
      ...
      Bun.spawnSync(["bun", "run", "src/main.ts", "create", "usage-test"], { env });

      // Create mock session files
      const sessionDir = join(dir, "usage-test", "claude", "claude-code-sessions");
      ...
      const result = Bun.spawnSync(["bun", "run", "src/main.ts", "usage"], { env });
      expect(result.exitCode).toBe(0);
  ...
  test("shows detail for specific profile", async () => {
    await withTempProfiles(async (dir) => {
      const env = { ...process.env, AGP_PROFILES_DIR: dir };
      Bun.spawnSync(["bun", "run", "src/main.ts", "create", "detail-test"], { env });

      const sessionDir = join(dir, "detail-test", "claude", "claude-code-sessions");
      await mkdir(sessionDir, { recursive: true });
      await Bun.write(
      ...
      const result = Bun.spawnSync(["bun", "run", "src/main.ts", "usage", "detail-test"], { env });
      expect(result.exitCode).toBe(0);
      const output = result.stderr.toString();
